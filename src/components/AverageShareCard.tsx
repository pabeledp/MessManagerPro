    ...
    <div className="glass-panel rounded-2xl p-4 sm:p-5 mb-4 border border-slate-200/80 shadow-sm relative overflow-hidden bg-white">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-bangla">
              {language === 'bn' ? `মাথাপিছু খরচ: ৳${avgExpensePerHead.toFixed(0)}` : `Cost / head: ৳${avgExpensePerHead.toFixed(0)}`}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        ...
        <div className="p-3 rounded-xl bg-slate-50 border border-slate-100">
          ...
          <span className="text-[9px] text-slate-400 block font-english mt-0.5">
        ...
        <div className="p-3 rounded-xl bg-emerald-50/60 border border-emerald-100">
          ...
          <span className="text-[9px] text-emerald-600 block font-english mt-0.5">
            ৳{totalDeposit.toLocaleString()} ÷ {activeMembers.length}
          </span>
        </div>
      </div>

      {/* Two Breakdown Cards: মোট বাকি vs মোট পাবে */}
      <div className="grid grid-cols-1 gap-2 mb-2.5">
        ...
        <div className="p-2.5 rounded-xl bg-rose-50/50 border border-rose-100">
          <div className="flex items-center justify-between pb-1.5 border-b border-rose-100 mb-1.5">
            <div className="flex items-center gap-1">
              ...
              <h3 className="text-xs font-black text-rose-800 font-bangla">
                {language === 'bn' ? 'মোট বাকি' : 'Total Dues'}
              </h3>
            </div>
          ...
          {paidLessMembers.length === 0 ? (
            <p className="py-1 text-center text-xs text-emerald-600 flex items-center justify-center gap-1 font-bangla font-bold">
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" /> {t.noDue}
            </p>
          ) : (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 font-bangla">
                        {language === 'bn' ? `জমা ৳${m.deposit} • খরচ ৳${m.cost.toFixed(0)}` : `Deposit ৳${m.deposit} • Cost ৳${m.cost.toFixed(0)}`}
                      </p>
                    </div>
                  </div>

                  <span className="px-2 py-0.5 rounded-md bg-rose-100 text-rose-800 font-black text-xs border border-rose-200 font-bangla">
          ...
          <div className="flex items-center justify-between pb-1.5 border-b border-emerald-100 mb-1.5">
            <div className="flex items-center gap-1">
              ...
              <h3 className="text-xs font-black text-emerald-800 font-bangla">
                {language === 'bn' ? 'মোট পাবে' : 'Total Surplus'}
              </h3>
            </div>
            <span className="text-xs font-black text-emerald-700 font-english">
              +৳{totalSurplus.toFixed(0)}
            </span>
          </div>

          {paidMoreMembers.length === 0 ? (
            <p className="py-1 text-center text-xs text-slate-400 font-bangla">{t.noSurplus}</p>
          ) : (
            <div className="space-y-1.5">
              {paidMoreMembers.map((m) => (
                ...
                >
                  ...
                  <span className="px-2 py-0.5 rounded-md bg-emerald-100 text-emerald-800 font-black text-xs border border-emerald-200 font-bangla">
